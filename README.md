...
Zoom · Google Meet · Teams · любое приложение со звуком. По умолчанию всё остаётся на вашем Mac.
...
| **Локально по умолчанию** | аудио, транскрипты, очистка и саммари — всё на вашем Mac. Выгрузка в общую базу — **только по явному согласию: после обработки приложение спрашивает «Отправить?»** |
| **Очистка + саммари** | по ключу ЛЛМ (в настройках) рядом с сырым транскриптом появляются очищенная версия (без личного и наездов на людей) и саммари с action items — считаются локально |
| **В базу — только очищенное** | если согласились отправить, в общую базу уходят очищенная версия и саммари, сырой транскрипт остаётся у вас |
...
| **Встречи стык в стык** | расшифровка идёт фоном и не мешает начать следующую запись сразу после «Стоп» |
| **Пауза** | ждёте, пока подключатся — пустое ожидание не попадает в транскрипт |
| **Имена вместо номеров** | голос узнаётся по библиотеке отпечатков: «Собеседник 3» становится именем |
| **Плавающий «Стоп»** | панель поверх всего, включая полноэкранный Zoom |
| **Календарь** | всплывашка «Подключиться и записать» + список участников в шапке транскрипта |
| **Своя папка записей** | где хранить аудио — выбираете в настройках |
...
## Имена вместо «Собеседник 3»

Метка «Собеседник 7» бесполезна: читатель не понимает, чьи это слова. Приложение
запоминает голоса и подставляет имена — **без ручной разметки**.

Разметка берётся со встреч один-на-один: календарь знает, что участников двое,
значит единственный чужой голос на записи принадлежит тому единственному человеку,
который есть в приглашении кроме вас. Дальше этот голос узнаётся на больших встречах.

Замерено на реальных записях: тот же человек в разных частях встречи — близость
**0.928**, разные люди — максимум **0.470**. Порог **0.75** стоит между ними с запасом.

Запоминание намеренно осторожное: отпечаток берётся, только если на «встрече на
двоих» найден **ровно один** чужой голос и он говорил не меньше минуты. Если
подключился третий — запись в библиотеку пропускается. Причина простая: ошибочный
отпечаток будет молча и уверенно подписывать чужие слова конкретным именем.

Библиотека лежит **на виду** — `~/Documents/Meeting Transcriber/voiceprints.json`.
Это биометрические данные ваших коллег, поэтому они не спрятаны в недрах системы:
удалить можно одним файлом. Ничего никуда не отправляется.

Восстановить имена в старых записях (данные встреч подтягиваются из календаря по
времени записи, звук должен быть сохранён):

```bash
python backfill.py            # подтянуть данные встреч (быстро, без GPU)
python backfill.py --rerun    # + перепрогнать диаризацию и переписать имена
```

## Выгрузка в общую базу (опционально)

Если команде нужен общий доступ к транскриптам, приложение умеет класть их в базу
(ClickHouse по HTTP). **По умолчанию выключено и ничего не отправляет.**

Как это устроено:

- креды вводит сам человек в **Настройках**; пароль хранится в **Keychain**, не в файлах;
- ни один транскрипт не уходит автоматически — вы **сами отмечаете конкретную встречу**;
- передумали — снимаете отметку, запись скрывается из общей выборки;
- автор проставляется самой базой (`currentUser()`), подделать нельзя.

Таблица (имя задаёте вы, схема — на ваше усмотрение):

```sql
CREATE TABLE <схема>.meeting_transcripts (
    meeting_key String,   title String,  started_at DateTime,
    uploaded_by String DEFAULT currentUser(),
    body String,          deleted UInt8 DEFAULT 0,
    created_at DateTime DEFAULT now()
) ENGINE = ReplacingMergeTree(created_at, deleted)
