...
2. Дважды кликните **`install.command`**. При первом запуске macOS может ругнуться — тогда правый клик по файлу → **Открыть** → **Открыть**.
3. Скрипт сам поставит Homebrew, ffmpeg, Python-окружение, соберёт и установит приложение, по шагам спросит токен для разделения спикеров. Дождитесь «Готово 🎉».

Нужен Mac на Apple Silicon (M1–M4). Качается несколько ГБ — первый раз это надолго.
